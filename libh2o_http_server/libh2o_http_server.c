...
#define NOTIFICATION_START_TIMER 3
#define NOTIFICATION_STOP_TIMER 4
...
struct server_context_t {
    struct http_server_init_t
        server_init; /* server init parameters from user */
 ...
 * MUST tbe called in event loop thread
 */
static void set_current_thread_index(struct server_context_t *c,
                                     int thread_index)
{
    struct server_tls_data_t *p;
...
static void server_tls_destroy(void *value) { free(value); }
...
static int add_lock_callback(int *num, int amount, int type, const char *file,
                             int line)
{
    (void)type;
    (void)file;
...
static void init_openssl(void)
{
    ...
    if (openssl_inited++ == 0) {
        ...
        for (i = 0; i != nlocks; ++i) pthread_mutex_init(mutexes + i, NULL);
...
static void callback_on_ws_sent(struct notification_ws_conn_t *conn, void *buf,
                                size_t len, int sent)
{
    struct server_context_t *c = conn->cmn.c;
    struct http_server_init_t *p = &c->server_init;
...
static void callback_on_ws_recv(struct notification_ws_conn_t *conn, void *buf,
                                size_t len)
{
    struct server_context_t *c = conn->cmn.c;
    struct http_server_init_t *p = &c->server_init;
...
static void callback_on_ws_closed(struct notification_ws_conn_t *conn,
                                  const char *err)
{
    struct server_context_t *c = conn->cmn.c;
    struct http_server_init_t *p = &c->server_init;
...
static void dispose_timeout_cb(h2o_timer_t *entry)
{
    ...
    conn = H2O_STRUCT_FROM_MEMBER(struct notification_ws_conn_t,
                                  dispose_timeout, entry);
                          ...
                          const struct wslay_event_on_msg_recv_arg *arg)
{
    ...
    if (arg == NULL) {
        ...
        h2o_timer_link(c->threads[thread_index].ctx.loop, DISPOSE_TIMEOUT_MS,
                       &conn->dispose_timeout);
        return;
    }
...
static void resp_timeout_cb(h2o_timer_t *entry)
{
    ...
    conn = H2O_STRUCT_FROM_MEMBER(struct notification_http_conn_t, _timeout,
                                  entry);
...
static struct listener_config_t *find_listener(struct server_context_t *c,
...
static struct listener_config_t *add_listener(struct server_context_t *c,
                                              int fd, struct sockaddr *addr,
                                              socklen_t addrlen, int is_global,
                                              int proxy_protocol)
{
    ...
    c->listeners = h2o_mem_realloc(c->listeners, sizeof(*(c->listeners)) *
                                                     (c->num_listeners + 1));
    c->listeners[c->num_listeners++] = listener;
    return listener;
}

static int open_tcp_listener(struct server_context_t *c, const char *hostname,
                             const char *servname, int domain, int type,
                             int protocol, struct sockaddr *addr,
                             socklen_t addrlen, int *so_reuseport)
{
    int fd;

    if ((fd = socket(domain, type, protocol)) == -1) goto Error;
    set_cloexec(fd);
    { /* set reuseaddr */
        int flag = 1;
...
#endif
    if (bind(fd, addr, addrlen) != 0) goto Error;
    if (listen(fd, H2O_SOMAXCONN) != 0) goto Error;
    ...
    while (*servname != NULL) {
        ...
        for (ai = res; ai != NULL; ai = ai->ai_next) {
            struct listener_config_t *listener =
                find_listener(c, ai->ai_addr, ai->ai_addrlen);
            ...
            if (listener == NULL) {
                ...
                so_reuseport = 1;
#endif
                if ((fd = open_tcp_listener(
                         c, hostname, *servname, ai->ai_family, ai->ai_socktype,
...
#if defined(__linux__) && defined(SO_REUSEPORT)
                /**
                 * socket open for all threads should before setuid() call when
                 * so_reuseport enabled
                 * in case of "user" has been configured.
                 * because: To prevent port hijacking all sockets bound to the
                 * same port using so_reuseport must have the
                 * same uid
                 */
                if (so_reuseport) {
                    size_t i;
                    listener->reuseport_fds =
                        h2o_mem_alloc(sizeof(int) * c->server_init.num_threads);
                    listener->reuseport_fds[0] = fd;
                    for (i = 1; i < c->server_init.num_threads; ++i) {
                        if ((fd = open_tcp_listener(
                                 c, hostname, *servname, ai->ai_family,
                                 ai->ai_socktype, ai->ai_protocol, ai->ai_addr,
...
static int setup_ssl(struct server_context_t *c, const char *cert_file,
                     const char *key_file, const char *ciphers)
{
    SSL_load_error_strings();
    SSL_library_init();
    OpenSSL_add_all_algorithms();
...
static void release_pending_data_linklist(struct notification_ws_conn_t *conn,
                                          int sent)
{
...
static void queue_websocket_data(struct notification_ws_conn_t *conn,
                                 struct notification_data_t *data)
{
    struct wslay_event_msg msgarg = {
        WSLAY_TEXT_FRAME, (const uint8_t *)data->data.base, data->data.len};
...
static int queue_websocket_broadcast_cb(struct notification_ws_conn_t *conn,
                                        void *cbdata)
{
    if (conn->wsconn != NULL) {
...
static int queue_websocket_close_cb(struct notification_ws_conn_t *conn,
                                    void *cbdata)
{
    if (conn->wsconn != NULL) {
...
static int foreach_ws_conn(h2o_linklist_t *list,
                           int (*cb)(struct notification_ws_conn_t *conn,
                                     void *cbdata),
                           void *cbdata)
{
    h2o_linklist_t *node;
    ...
    for (node = list->next; node != list; node = node->next) {
        struct notification_ws_conn_t *conn =
            (struct notification_ws_conn_t *)(node);
...
static void process_ws_broadcast(struct server_context_t *c, int thread_index,
                                 struct notification_data_t *data)
{
    foreach_ws_conn(&c->threads[thread_index].ws_conns,
                    queue_websocket_broadcast_cb, data);
}

static void queue_ws_connection_close(struct server_context_t *c,
                                      int thread_index)
{
    foreach_ws_conn(&c->threads[thread_index].ws_conns,
...
static void release_timers_linkedlist(struct server_context_t *c,
...
find_notification_timer(struct server_context_t *c, uint32_t id)
{
    h2o_linklist_t *messages = &c->threads[0].timers;
    ...
    while (1) {
        ...
        if (messages == &c->threads[0].timers) break;
    }
    return NULL;
}
...
static void user_timeout_cb(h2o_timer_t *entry)
{
    ...
    if (timer->timer.flags & LIBH2O_EVLOOP_TIMER_REPEAT) {
        h2o_timer_link(timer->cmn.c->threads[0].ctx.loop,
                       timer->timer.timeout_ms, &timer->_timeout);
    } else {
        release_notification_timer(timer);
    }
}
                                   ...
                                   h2o_linklist_t *messages)
{
    while (!h2o_linklist_is_empty(messages)) {
        ...
        if (0) {
            ...
            struct notification_start_timer_t *timer =
                (struct notification_start_timer_t *)msg;
            h2o_linklist_insert(&c->threads[0].timers, &msg->link);
            timer->_timeout.cb = user_timeout_cb;
            h2o_timer_link(c->threads[0].ctx.loop, timer->timer.timeout_ms,
                           &timer->_timeout);
...
static void *server_loop(void *_param)
{
    ...
    h2o_context_init(&c->threads[thread_index].ctx, h2o_evloop_create(),
                     &c->config);
    h2o_multithread_register_receiver(
        c->threads[thread_index].ctx.queue,
        &c->threads[thread_index].server_notifications, on_server_notification);
#if USE_MEMCACHED
    h2o_multithread_register_receiver(c->threads[thread_index].ctx.queue,
                                      &c->threads[thread_index].memcached,
    ...
    for (i = 0; i != c->num_listeners; ++i) {
        ...
        listeners[i].accept_ctx.expect_proxy_line =
            listener_config->proxy_protocol;
#if USE_MEMCACHED
        listeners[i].accept_ctx.libmemcached_receiver =
            &c->threads[thread_index].memcached;
#else
        listeners[i].accept_ctx.libmemcached_receiver = NULL;
#endif
        listeners[i].sock = h2o_evloop_socket_create(
            c->threads[thread_index].ctx.loop, fd, H2O_SOCKET_FLAG_DONT_READ);
    ...
    while (1) {
        if (c->threads[thread_index].exit_loop) break;
    ...
    h2o_multithread_unregister_receiver(
        c->threads[thread_index].ctx.queue,
        &c->threads[thread_index].server_notifications);
#if USE_MEMCACHED
    h2o_multithread_unregister_receiver(c->threads[thread_index].ctx.queue,
                                        &c->threads[thread_index].memcached);
#endif
...
static void free_server_thread_data(struct server_context_t *c)
{
    if (c->threads) {
        int i;
        for (i = 0; i < c->server_init.num_threads; ++i) {
            release_timers_linkedlist(c, &c->threads[i].timers);
        }
        free(c->threads);
        c->threads = NULL;
    }
...
const char *libh2o_http_server_get_version(void) { return H2O_VERSION; }
...
libh2o_http_server_start(const struct http_server_init_t *server_init)
{
    ...
    hostconf = h2o_config_register_host(
        &c->config, h2o_iovec_init(H2O_STRLIT("default")), 65535);
    pathconf = h2o_config_register_path(hostconf, "/", 0);

    handler = (struct server_handler_t *)h2o_create_handler(pathconf,
                                                            sizeof(*handler));
    ...
    c->threads =
        h2o_mem_alloc(server_init->num_threads * sizeof(c->threads[0]));
    ...
    for (i = 0; i < c->server_init.num_threads; ++i) {
        ...
        h2o_linklist_init_anchor(&c->threads[i].timers);
    }
...
static void notify_thread_resp(struct notification_http_conn_t *conn)
{
    ...
    h2o_multithread_send_message(&c->threads[thread_index].server_notifications,
                                 &conn->cmn.super);
}
                                    ...
                                    const void *buf, size_t len)
{
    ...
    if (H2O_UNLIKELY(clih == NULL)) return 0;
...
static void notify_thread_broadcast(struct server_context_t *c, const void *buf,
                                    size_t len)
{
    size_t i;
...
size_t libh2o_http_server_broadcast_ws_message(struct server_context_t *c,
                                               const void *buf, size_t len)
{
    if (H2O_UNLIKELY(c == NULL)) return 0;
...
static struct notification_start_timer_t *
notify_thread_start_timer(struct server_context_t *c,
                          struct libh2o_evloop_timedout_t *to,
                          uint32_t timeout_ms, uint32_t flags)
{
    ...
    h2o_multithread_send_message(&c->threads[0].server_notifications,
                                 &msg->cmn.super);
    return msg;
}

static void notify_thread_stop_timer(struct server_context_t *c, uint32_t id)
{
