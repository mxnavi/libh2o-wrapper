...
static uint16_t __max_buffer_size(const char *env)
{
    if (env) {
        return (uint16_t)int_from_config_string(env, "max_buffer_size=", 0);
    }
    return 0;
}
...
#endif
{
    ...
    client_init.max_buffer_size = __max_buffer_size(env);
